...
# یک وب‌سرور بسیار ساده برای تایید سلامت رندر
...
async def global_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # اگر پیام از طرف ادمین باشد و روی پیامی ریپلای کرده باشد
    if user_id == ADMIN_ID:
    ...
    # اگر مشتری پیام متنی پشتیبانی بفرستد
...
async def start_bot():
    ...
    # متد استاندارد برای اجرای طولانی مدت ربات روی سرورهای ابری
    await app.initialize()
    await app.start()
    await app.updater.start_polling(drop_pending_updates=True)
    
    # زنده نگه داشتن لوپ اصلی پایتون
    while True:
        await asyncio.sleep(3600)

if __name__ == "__main__":
    # ۱. راه اندازی فوری وب‌سرور پورت در یک ترد جداگانه
