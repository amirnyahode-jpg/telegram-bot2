...
# ساخت وب‌سرور بسیار سبک برای چراغ سبز رندر (رفع خطای پورت و ارور ۴۰۴)
...
# مدیریت پیام‌های دریافتی (مشتری و ادمین)
async def global_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ۱. بخش ادمین: پاسخ دادن به مشتری با ریپلای روی رسید یا پیام
    if user_id == ADMIN_ID:
        if update.message.reply_to_message:
            try:
                ...
                if target_text and "User ID:" in target_text:
                    ...
                    # کپی دقیق هر نوع پیامی که ادمین فرستاده برای مشتری
                    ...
                    await update.message.reply_text("❌ خطا: اطلاعات کاربر پیدا نشد. روی پیام ارسال شده توسط ربات ریپلای کنید.")
            except Exception as e:
                await update.message.reply_text(f"❌ خطا در ارسال: {e}")
        return

    # ۲. بخش مشتری: فرستادن عکس (رسید) برای ادمین
    ...
    # ۳. بخش مشتری: فرستادن پیام متنی پشتیبانی
...
async def start_bot():
    ...
    print("ربات تلگرام آماده به کار است...")
    
    await app.initialize()
    await app.start()
    # پاک کردن آپدیت‌های قدیمی تلگرام برای جلوگیری از تداخل
...
if __name__ == "__main__":
    # اجرای وب‌سرور روی پورت در ترد فرعی
